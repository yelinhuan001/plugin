...
// Simplified for iOS arm64 / iOS 14 巨魔 dylib 插件
...
static int get_protection(void *addr, vm_prot_t *prot, vm_prot_t *max_prot) {
  ...
  if (info_ret == KERN_SUCCESS) {
    if (prot != NULL) *prot = info.protection;
    if (max_prot != NULL) *max_prot = info.max_protection;
    return 0;
  }
  return -1;
}
        ...
        if (symbol_name_longer_than_1 &&
          ...
          if (err == KERN_SUCCESS) {
            ...
            void *replacement = ptrauth_sign_unauthenticated(
                cur->rebindings[j].replacement, ptrauth_key_function_pointer, 0);
#else
